...
**Database**: PostgreSQL 16 (`postgres:16-alpine` in Compose), one database named `shortener`, owned by the `shortener` role. The schema is defined as SQLAlchemy ORM models in `shortener-service/shortener_service/db/models.py` and applied through versioned Alembic migrations — the application never creates tables itself.

### Table `urls` — the single domain table, owned by `shortener-service`
...
| Column | Type | Constraints | Purpose and rationale |
|---|---|---|---|
| `id` | `BIGINT` identity | **Primary key** (`urls_pkey`) | Surrogate key, auto-generated by the database. `BIGINT` (not `INT`) so the keyspace never needs a painful migration; identity column (not app-generated) so inserts stay a single round trip. |
| `alias` | `VARCHAR(64)` | `NOT NULL`, **unique** via `ix_urls_alias` | The short code in the URL. Custom aliases are validated to `^[a-zA-Z0-9_-]{3,32}$` (service layer); generated aliases are 7 base62 chars; 64 leaves headroom to relax the policy without a schema change. |
| `long_url` | `TEXT` | `NOT NULL` | The destination. `TEXT` not `VARCHAR(n)`: URLs legitimately run long, and in PostgreSQL `TEXT` has no performance penalty over `VARCHAR`; length/shape policy is enforced at the API layer (Pydantic `HttpUrl`: valid http/https, max 2083 chars) where it can return a friendly 422. |
| `is_custom` | `BOOLEAN` | `NOT NULL DEFAULT false` | Whether the alias was user-chosen or generated; returned in metadata, useful for analytics/abuse review. |
| `created_at` | `TIMESTAMPTZ` | `NOT NULL DEFAULT now()` | Creation time, stamped by the database (one clock source, not N app replicas' clocks). `TIMESTAMPTZ` everywhere — timestamps are stored UTC and never ambiguous. |
| `expires_at` | `TIMESTAMPTZ` | `NULL` allowed | TTL extension: absolute expiry computed at creation (`now + ttl_seconds`); `NULL` means the link never expires. Enforced lazily by the read query (below), not by a scheduler. |
| `access_count` | `BIGINT` | `NOT NULL DEFAULT 0` | Click counter, incremented by the analytics-worker in batches via atomic `UPDATE ... SET access_count = access_count + :n`. `BIGINT` because popular links outgrow `INT`'s 2.1 billion. |

### Keys and indexes

| Index | Definition | Role |
|---|---|---|
| `urls_pkey` | `PRIMARY KEY (id)`, btree | Row identity; used by ORM operations. |
| `ix_urls_alias` | `UNIQUE (alias)`, btree | Does double duty: O(log n) lookups on the redirect hot path (`WHERE alias = :alias`), **and** the conflict arbiter for `INSERT ... ON CONFLICT (alias) DO NOTHING` — the database-enforced guarantee that two concurrent claims of the same alias produce exactly one winner. This index existing is what makes the whole concurrency story (section 5) work. |

No foreign keys exist — there is only one table. The redirect path reads it with the expiry predicate baked in:
...
### Table `alembic_version` — migration bookkeeping (owned by Alembic)

| Column | Type | Constraints | Purpose |
|---|---|---|---|
| `version_num` | `VARCHAR(32)` | Primary key (`alembic_version_pkc`) | The single row records which migration the schema is at; Alembic reads it on `alembic upgrade head` to decide what to apply. |

Migration history: `0001` creates `urls` + `ix_urls_alias`; `0002` adds `expires_at`. Migrations run automatically on shortener-service startup (its container entrypoint runs `alembic upgrade head` before the API starts; concurrent replicas are safe because Alembic takes a lock on the version table).
